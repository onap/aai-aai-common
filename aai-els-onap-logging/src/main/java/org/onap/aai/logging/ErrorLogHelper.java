...
public class ErrorLogHelper {
    ...
    // TODO: let this return a RestApiErrorResponse object that both Service and Policy inherit from
    public static Fault getRESTAPIErrorResponse2(AAIException aaiException,
        ...
        // return createServiceErrorResponse(variables, text, aaiException);
        return new Fault();
    }
    ...
    public static String getRESTAPIErrorResponse(List<MediaType> acceptHeaders, AAIException aaiException,
        ...
        for (MediaType mediaType : validAcceptHeaders) {
            if (MediaType.APPLICATION_XML_TYPE.isCompatible(mediaType)) {
                try {
                    return createExceptionResponseXml(variables, aaiException, text);
                } catch (Exception ex) {
                    LOGGER.error(
                ...
                try {
                    return createExceptionResponseJson(variables, aaiException, text);
                } catch (Exception ex) {
                    LOGGER.error(
                            "We were unable to create a rest exception to return on an API because of a parsing error "
                                    + ex.getMessage());
    ...
    public static String getRESTAPIPolicyErrorResponseXML(AAIException aaiException, ArrayList<String> variables) {
        ...
        int restErrorCode = Integer.parseInt(aaiException.getErrorObject().getRESTErrorCode());
    ...
    private static String createExceptionResponseXml(ArrayList<String> variables, final AAIException aaiException,
            final String text) throws JAXBException, JsonProcessingException {
        if (aaiException.getErrorObject().getCategory().equals("1")) {
            // org.onap.aai.domain.restPolicyException.Fault fault = createPolicyExceptionFault(variables, aaiException.getErrorObject(),
            //         text);
            Fault fault = createPolicyFault(aaiException, text, variables);
            return xmlMapper.writeValueAsString(fault);
            // return marshallXml(fault);
        } else {
            // org.onap.aai.domain.restServiceException.Fault fault = createServiceExceptionFault(variables,
            //         aaiException.getErrorObject(), text);
            Fault fault = createServiceFault(aaiException, text, variables);
            return xmlMapper.writeValueAsString(fault);
            // return marshallXml(fault);
        }
    }

    private static String createExceptionResponseJson(ArrayList<String> variables, AAIException aaiException,
            final String text) throws AAIException, JsonProcessingException {
        if (aaiException.getErrorObject().getCategory().equals("1")) {
            Fault fault = createPolicyFault(aaiException, text, variables);
            return objectMapper.writeValueAsString(fault);
            // RESTResponse policyRESTResponse = createPolicyRESTResponse(variables, text, aaiException);
            // return objectMapper.writeValueAsString(policyRESTResponse);
        } else {
            Fault fault = createServiceFault(aaiException, text, variables);
            return objectMapper.writeValueAsString(fault);
            // org.onap.aai.domain.restServiceException.RESTResponse serviceRESTResponse = createServiceErrorResponse(variables, text, aaiException);
            // return objectMapper.writeValueAsString(serviceRESTResponse);
        }
    }

    private static org.onap.aai.domain.restPolicyException.Fault createPolicyExceptionFault(ArrayList<String> variables,
            final ErrorObject errorObject, final String text) {
    ...
    private static org.onap.aai.domain.restServiceException.RESTResponse createServiceErrorResponse(ArrayList<String> variables, String text, AAIException aaiException) {
        ...
        serviceException.setMessageId("SVC" + aaiException.getErrorObject().getRESTErrorCode());
        serviceException.setText(text);
    ...
    private static RESTResponse createPolicyRESTResponse(List<String> variables, String text, AAIException aaiException)
            throws AAIException {
    ...
    private static org.onap.aai.domain.restPolicyException.RequestError createPolicyRequestError(List<String> variables,
            String text, AAIException aaiException) {
        ...
        policyException.setMessageId("POL" + aaiException.getErrorObject().getRESTErrorCode());
        policyException.setText(text);
