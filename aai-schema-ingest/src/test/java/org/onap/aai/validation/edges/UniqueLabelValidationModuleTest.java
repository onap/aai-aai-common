...
public class UniqueLabelValidationModuleTest {
    ...
    public void testInvalidSetOneFileBothTypes() {
        assertTrue(validator.validate("sphinx|monster", ctxs)
            .contains("has multiple rules using the same label"));
        assertTrue(validator.validate("monster|sphinx", ctxs)
            .contains("has multiple rules using the same label"));
    }

    @Test
    public void testInvalidSetOneFileJustCousins() {
        assertTrue(validator.validate("griffin|hippogriff", ctxs)
            .contains("has multiple rules using the same label"));
        assertTrue(validator.validate("hippogriff|griffin", ctxs)
            .contains("has multiple rules using the same label"));
    }

    @Test
    public void testInvalidSetMultipleFiles() {
        assertTrue(validator.validate("lava|floor", ctxs)
            .contains("has multiple rules using the same label"));
        assertTrue(validator.validate("floor|lava", ctxs)
            .contains("has multiple rules using the same label"));
    }

    @Test
    public void testInvalidCopyInOtherFile() {
        assertTrue(validator.validate("badger|mushroom", ctxs)
            .contains("has multiple rules using the same label"));
    }
}
