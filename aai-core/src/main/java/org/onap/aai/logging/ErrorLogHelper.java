...
public class ErrorLogHelper {
    ...
    private static final EELFLogger LOGGER =
        EELFManager.getInstance().getLogger(ErrorLogHelper.class);
    private static final HashMap<String, ErrorObject> ERROR_OBJECTS =
        new HashMap<String, ErrorObject>();
    ...
    /**
     * Determines whether category is policy or not. If policy (1), this is a POL error, else it's a
     * SVC error.
     * The AAIRESTException may contain a different ErrorObject than that created with the REST
     * error key.
     * This allows lower level exception detail to be returned to the client to help troubleshoot
     * the problem.
     * If no error object is embedded in the AAIException, one will be created using the error
     * object from the AAIException.
     * 
     * @param are must have a restError value whose numeric value must match what should be returned
     *        in the REST API
     ...
     * @throws IOException
     */
    public static String getRESTAPIErrorResponse(List<MediaType> acceptHeadersOrig,
        AAIException are, ArrayList<String> variables) {
        ...
        try {
            ...
            LOGGER.warn("Failed to find related error object AAI_" + restErrorCode
                + " for error object " + eo.getErrorCode() + "; using AAI_" + restErrorCode);
            restErrorObject = eo;
        }
        ...
        // This assumes that the variables in the ArrayList, which might be more than are needed to
        // flesh out the
        // error, are ordered based on the error string.
        int localDataIndex = StringUtils.countMatches(restErrorObject.getErrorText(), "%");
        text.append(" (msg=%").append(localDataIndex + 1).append(") (ec=%")
            .append(localDataIndex + 2).append(")");
            ...
            if (MediaType.APPLICATION_XML_TYPE.isCompatible(mediaType)) {
                ...
                try {
                    if (eo.getCategory().equals("1")) {
                        ...
                        context = JAXBContext
                            .newInstance(org.onap.aai.domain.restPolicyException.Fault.class);
                        ...
                        context = JAXBContext
                            .newInstance(org.onap.aai.domain.restServiceException.Fault.class);
                        Marshaller m = context.createMarshaller();
                        m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
                        m.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
                        ...
                        org.onap.aai.domain.restServiceException.Fault fault =
                            factory.createFault();
     ...
     * @throws IOException
     */
    public static String getRESTAPIErrorResponseWithLogging(List<MediaType> acceptHeadersOrig,
        AAIException are, ArrayList<String> variables) {
    ...
    public static Object getRESTAPIInfoResponse(List<MediaType> acceptHeaders,
        ...
        while (it.hasNext()) {
            Entry<AAIException, ArrayList<String>> pair =
                (Entry<AAIException, ArrayList<String>>) it.next();
            ...
            try {
                restErrorObject =
                    ErrorLogHelper.getErrorObject("AAI_" + String.format("%04d", restErrorCode));
            } catch (ErrorObjectNotFoundException e) {
                restErrorObject = eo;
            }
            text.append(restErrorObject.getErrorText());

            // We want to always append the (msg=%n) (ec=%n+1) to the text, but have to find value
            // of n
            // This assumes that the variables in the ArrayList, which might be more than are needed
            // to flesh out the
            // error, are ordered based on the error string.
            int localDataIndex = StringUtils.countMatches(restErrorObject.getErrorText(), "%");
            text.append(" (msg=%").append(localDataIndex + 1).append(") (rc=%")
                .append(localDataIndex + 2).append(")");
     ...
     * @throws IOException
     */
    public static String getRESTAPIPolicyErrorResponseXML(AAIException are,
        ...
        try {
            if (eo.getCategory().equals("1")) {
                ...
                context =
                    JAXBContext.newInstance(org.onap.aai.domain.restPolicyException.Fault.class);
                Marshaller m = context.createMarshaller();
                m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
                m.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
                ...
                context =
                    JAXBContext.newInstance(org.onap.aai.domain.restServiceException.Fault.class);
                Marshaller m = context.createMarshaller();
                m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
                m.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
    ...
    public static void logException(AAIException e) {
        ...
        // MDC.put("severity", errorObject.getSeverity()); //TODO Use LoggingContext.severity(int
        // severity)
        ...
        final String errorMessage = new StringBuilder().append(errorObject.getErrorText())
            .append(":").append(errorObject.getRESTErrorCode()).append(":")
            .append(errorObject.getHTTPResponseCode()).append(":").append(e.getMessage()).toString()
            .replaceAll("\\n", "^");

        LoggingContext
            .responseCode(Integer.toString(errorObject.getHTTPResponseCode().getStatusCode()));
        ...
        final String details = new StringBuilder().append(errorObject.getErrorCodeString())
            .append(" ").append(stackTrace).toString();
