...
public class PojoUtils {
    ...
    public <T> List<KeyValueList> getKeyValueList(Entity e, T clazz)
        ...
        for (Method method : methods) {
            if (method.getName().startsWith("get")) {
                propertyName = CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_HYPHEN,
     ...
     */
    public <T> String getJsonFromObject(T clazz)
     ...
     */
    public String getJsonFromDynamicObject(DynamicEntity ent,
        org.eclipse.persistence.jaxb.JAXBContext jaxbContext, boolean includeRoot)
     ...
     */
    public String getLookupKey(String baseKey, HashMap<String, Object> lookupHash,
    ...
    public <T> void getExampleObject(T clazz, boolean singleton)
        throws IllegalAccessException, IllegalArgumentException, InvocationTargetException,
        NoSuchMethodException, SecurityException, AAIException {
        Method[] methods = clazz.getClass().getDeclaredMethods();
        ...
        for (Method method : methods) {
            boolean go = false;
            if (method.getName().startsWith("get")) {
                dnHypPropertyName = CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_HYPHEN,
                    method.getName().substring(3));
                upCamPropertyName = CaseFormat.UPPER_CAMEL.to(CaseFormat.UPPER_CAMEL,
                    method.getName().substring(3));
                go = true;
            ...
            if (go) {
                ...
                if (!retType.contains("aai") && !retType.contains("java.util.List")) {
                    // get the setter
                    Method meth = clazz.getClass().getMethod("set" + upCamPropertyName,
     ...
     */
    public void getDynamicExampleObject(DynamicEntity childObject, AAIResource aaiRes,
        boolean singleton) {
        ...
        for (String dnHypAttrName : aaiRes.getStringFields()) {
            ...
            String dnCamAttrName =
                CaseFormat.LOWER_HYPHEN.to(CaseFormat.LOWER_CAMEL, dnHypAttrName);
