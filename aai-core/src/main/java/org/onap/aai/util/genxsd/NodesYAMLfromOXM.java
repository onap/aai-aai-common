...
public class NodesYAMLfromOXM extends OxmFileProcessor {
    ...
    private String processJavaTypeElementSwagger(String javaTypeName, Element javaTypeElement,
        ...
        for (int i = 0; i < xmlElementNodes.getLength(); ++i) {
            ...
            } else if (indexedProps == null || indexedProps.isEmpty()
                && "true".equals(xmlElementElement.getAttribute("required"))) {
                // no indexedProps and element is required, also considered using xml-key in this
                // case
                containerProps
                    .add(xmlElementElement.getPathParamYAMLRqd(elementDescription, false));
                NodeGetOperation.addContainerProps(container, containerProps);
            }
