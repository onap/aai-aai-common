...
public class XSDElement implements Element {
    ...
    public String getQueryParamYAML(String elementDescription) {
        ...
        if (elementDescription != null && StringUtils.isNotBlank(this.getAttribute("name"))) {
            sbParameter.append("          example: " + "__"
                + this.getAttribute("name").toUpperCase() + "__" + "\n");
        }
    ...
    public String getHTMLElement(SchemaVersion v, boolean useAnnotation, HTMLfromOXM driver) {
        ...
        if (addType != null) {
            sbElement.append(
                "        <xs:element ref=\"tns:" + driver.getXmlRootElementName(addType) + "\"");
        } else {
    ...
    public String getHTMLAnnotation(String target, String indentation) {
        ...
        sb.append(indentation + "  <xs:appinfo>" + OxmFileProcessor.LINE_SEPARATOR + indentation
            + "    <annox:annotate target=\"" + target + "\">@org.onap.aai.annotations.Metadata("
            + Joiner.on(",").join(metadata) + ")</annox:annotate>" + OxmFileProcessor.LINE_SEPARATOR
