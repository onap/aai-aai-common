        ...
        if (!endsInContainer) {
            ...
            if (!(parentName.equals("") || parentName.equals(this.resultResource))) {
                URI parentUri = UriBuilder
                    .fromPath(
                        uri.getRawPath().substring(0, uri.getRawPath().indexOf(containerResource)))
                    .build();
                this.dbKeyParser = new URIToDBKey(loader, parentUri);
                this.parentQueryBuilder = queryBuilder.newInstance().getVerticesByIndexedProperty(
                    "aai-unique-key", (String) dbKeyParser.getResult());
                this.parentResourceType = parentName;
            }
            this.containerResource = "";
        } else {
            URI parentUri = UriBuilder.fromPath(uri.getRawPath().substring(0,
                uri.getRawPath().indexOf(this.finalContainer.getDbName()))).build();
            this.dbKeyParser = new URIToDBKey(loader, parentUri);
            String dbKey = (String) dbKeyParser.getResult();
            this.parentResourceType = parentName;
