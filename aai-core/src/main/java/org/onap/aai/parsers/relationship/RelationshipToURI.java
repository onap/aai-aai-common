...
public class RelationshipToURI {
    ...
    private static final EELFLogger LOGGER =
        EELFManager.getInstance().getLogger(RelationshipToURI.class);
    ...
    protected void parse() throws AAIException {
        ...
        try {
            ...
            throw new AAIIdentityMapParseException(
                "Could not parse relationship-list object: " + e.getMessage(), e);
        }

    }

    private Optional<URI> processRelationshipData()
        ...
        for (Object datum : data) {
            ...
            try {
                ...
                if (!wrappedObj.hasProperty(propertyName)) {
                    throw new AAIIdentityMapParseException(
                        "invalid property name in map: " + propertyName);
                }
                ...
                if (wrappedObj.getMetadata(ObjectMetadata.NAMESPACE) != null) {
                    if (topLevelType == null) {
                        ...
                        throw new AmbiguousMapAAIException(
                            "found two top level nodes of different types: " + topLevelType
                                + " and " + objectType);
                    }
                }
            } catch (AAIUnknownObjectException e) {
                throw new AAIIdentityMapParseException("invalid object name in map: " + objectType,
                    e);
            }
    ...
    private Optional<URI> processRelatedLink(String relatedLink)
        ...
        if (relatedLink != null) {
            ...
            try {
                ...
                throw new AAIIdentityMapParseException("related link is invalid: " + relatedLink,
                    e);
            }
    ...
    private boolean sortRelationships(List<String> data, String startType, int i)
        ...
        for (j = (data.size() - i) - 1; j >= 0; j--) {
            ...
            try {
                rule = edgeRules.getRule(new EdgeRuleQuery.Builder(startType, objectType)
                    .edgeType(EdgeType.TREE).build());
                direction = rule.getDirection();
                if (direction != null) {
                    if ((rule.getContains().equals(AAIDirection.OUT.toString())
