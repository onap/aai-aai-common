...
public class InMemoryGraph {
    ...
    private static final EELFLogger LOGGER =
        EELFManager.getInstance().getLogger(InMemoryGraph.class);
        ...
        try {
            ...
            if (builder.isSchemaEnabled) {
                ...
                SchemaGenerator.loadSchemaIntoJanusGraph(graph, graphMgt,
                    graphProps.getProperty("storage.backend"));
            }
            ...
            if (builder.isPartialGraph) {
                if ((builder.graphsonLocation != null) && (builder.graphsonLocation.length() > 0)) {
                    ...
                    transaction.io(GraphSONPartialIO.build()).reader().create()
                        .readGraph(builder.seqInputStream, graph);
                }
            } else {
                if ((builder.graphsonLocation != null) && (builder.graphsonLocation.length() > 0)) {
                    transaction.io(IoCore.graphson()).readGraph(builder.graphsonLocation);
                } else {
                    transaction.io(IoCore.graphson()).reader().create()
                        .readGraph(builder.seqInputStream, graph);
                }
            }
            transaction.commit();
    ...
    public static class Builder {
        ...
        public InMemoryGraph build(String graphsonFile, String propertyFile,
            boolean isSchemaEnabled) throws IOException {
        ...
        public InMemoryGraph build(String graphsonFile, String propertyFile,
            boolean isSchemaEnabled, boolean isPartialGraph) throws IOException {
            this.graphsonLocation = graphsonFile;
            this.propertyFile = propertyFile;
            this.isSchemaEnabled = isSchemaEnabled;
