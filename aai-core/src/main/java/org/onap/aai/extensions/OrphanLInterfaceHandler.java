...
public class OrphanLInterfaceHandler {
    ...
    private QueryBuilder<Vertex> createLInterfaceQuery(AAIExtensionMap aaiReqMap,
        Introspector newvceObj) throws AAIException {
        Introspector uplinkLInterfaceTraversalIntro =
            aaiReqMap.getLoader().introspectorFromName("l-interface");

        Introspector customerUplinkLInterfaceTraversalIntro =
            aaiReqMap.getLoader().introspectorFromName("l-interface");

        Introspector logLinkIntroForTraversal =
            aaiReqMap.getLoader().introspectorFromName("logical-link");

        QueryBuilder<Vertex> query =
            aaiReqMap.getTransactionalGraphEngine().getQueryBuilder().exactMatchQuery(newvceObj)
                .createEdgeTraversal(EdgeType.TREE, newvceObj, uplinkLInterfaceTraversalIntro)
                .getVerticesByProperty("interface-role", "UPLINK")
                .createEdgeTraversal(EdgeType.COUSIN, uplinkLInterfaceTraversalIntro,
                    logLinkIntroForTraversal)
                .createEdgeTraversal(EdgeType.COUSIN, logLinkIntroForTraversal,
                    customerUplinkLInterfaceTraversalIntro)
    ...
    private URI buildLInterfaceURI(Vertex linterface, AAIExtensionMap aaiReqMap)
        ...
        QueryBuilder<Vertex> lagIntQuery =
            aaiReqMap.getTransactionalGraphEngine().getQueryBuilder().exactMatchQuery(lint)
        ...
        lagInterfaceForTrav.setValue("interface-name",
            (String) lagInt.property("interface-name").value());
    ...
    public List<DBRequest> createOrphanLInterfaceDelRequests(AAIExtensionMap aaiReqMap,
        Introspector newvce) throws AAIException, UnsupportedEncodingException, URISyntaxException {
        ...
        for (Vertex lint : linterfaces) {
            ...
            QueryParser parser =
                createLInterfaceQuery(aaiReqMap, newvce).createQueryFromObjectName("l-interface");
            DBRequest originalDbRequest = aaiReqMap.getDbRequest();
            DBRequest request = new DBRequest.Builder(HttpMethod.DELETE, lintURI, parser, newvce,
                originalDbRequest.getHeaders(), originalDbRequest.getInfo(),
                originalDbRequest.getTransactionId()).build();
