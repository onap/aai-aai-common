...
public final class AAIAuthCore {
    ...
    private synchronized void reloadUsers() {
        ...
        try {
            ...
            ErrorLogHelper.logError(ERROR_CODE_AAI_4001,
                globalAuthFileName + ". Not valid JSON: " + e);
        } catch (Exception e) {
            ErrorLogHelper.logError(ERROR_CODE_AAI_4001,
                globalAuthFileName + ". Exception caught: " + e);
        }
    }
    ...
    private Map<String, Boolean> getUsernamesFromRole(JsonObject roleObject)
            ...
            if (je.isJsonObject()) {
                if (je.getAsJsonObject().has("username")) {
                    if (je.getAsJsonObject().has("is-wildcard-id")) {
                        usernames.put(
                            je.getAsJsonObject().get("username").getAsString().toLowerCase(),
                    ...
                    String authorizationCode =
                        new String(Base64.base64Encode(auth.getBytes("utf-8")));
     ...
     */
    public boolean authorize(String username, String uri, String httpMethod, String haProxyUser,
        String issuer) throws AAIUnrecognizedFunctionException {
        ...
        if (oau.isPresent()) {
            ...
            if (au.hasRole("HAProxy")) {
                ...
                if ("GET".equalsIgnoreCase(httpMethod) && "util".equalsIgnoreCase(aaiMethod)
                    && haProxyUser.isEmpty()) {
            ...
            LOGGER.debug("User not found: " + username + " on function " + aaiMethod
    ...
    private boolean isWildcardIssuer(String issuer) {
        if (issuer != null && !issuer.isEmpty()) {
            List<String> validIssuers = Arrays.asList(AAIConfig
                .get("aaf.valid.issuer.wildcard", UUID.randomUUID().toString()).split("\\|"));
    ...
    public Optional<AAIUser> getUser(String username, boolean wildcardCheck) {
        if (users.containsKey(username)) {
            ...
            List<AAIUser> laus = users.entrySet().stream()
                .filter(e -> e.getValue().isWildcard() && username.contains(e.getKey()))
    ...
    private boolean authorize(AAIUser aaiUser, String aaiMethod, String httpMethod) {
        if (aaiUser.hasAccess(aaiMethod, httpMethod)) {
            ...
            LOGGER.debug("AUTH ACCEPTED: " + aaiUser.getUsername() + " on function " + aaiMethod
                + " request type " + httpMethod);
            return true;
        } else {
            LoggingContext.statusCode(StatusCode.ERROR);
            LOGGER.debug("AUTH FAILED: " + aaiUser.getUsername() + " on function " + aaiMethod
                + " request type " + httpMethod);
            return false;
        }
    }
