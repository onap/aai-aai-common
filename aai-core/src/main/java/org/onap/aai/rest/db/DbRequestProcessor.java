...
/**
 * Stateless processor for a batch of {@link DBRequest}s against a bound graph session.
 * Replaces the request-processing half of the former {@code HttpEntry}. A single singleton
 * instance is safe to share: all per-request state is derived from the {@link GraphBinding}
 * and {@link ProcessOptions} arguments, and transactions are owned by the caller.
 */
public class DbRequestProcessor {

    private static final Logger LOGGER = LoggerFactory.getLogger(DbRequestProcessor.class);
    ...
    public DbRequestProcessor(NotificationService notificationService, XmlFormatTransformer xmlFormatTransformer,
            SchemaVersions schemaVersions, LoaderFactory loaderFactory, String basePath) {
    ...
    @Observed(name = "DbRequestProcessor.process")
    public ProcessResult process(GraphBinding binding, List<DBRequest> requests, ProcessOptions options)
            throws AAIException {

        int notificationDepth = options.notificationDepth();
        UEBNotification notification = options.notification() != null
                ? options.notification()
                : new UEBNotification(loaderFactory, schemaVersions);

        DBSerializer serializer = null;
        if (binding.serverBase() != null) {
            serializer = new DBSerializer(binding.version(), binding.dbEngine(), binding.modelType(),
                    options.sourceOfTruth(), options.groups(), notificationDepth, binding.serverBase());
        } else {
            serializer = new DBSerializer(binding.version(), binding.dbEngine(), binding.modelType(),
                    options.sourceOfTruth(), options.groups(), notificationDepth);
        }
        ...
        QueryEngine queryEngine = binding.dbEngine().getQueryEngine();
        ...
        for (DBRequest request : requests) {
            ...
            try {
                try {
                    ...
                    if (options.queryOptions() != null && options.queryOptions().getPageable() != null) {
                        paginationResult = executePaginatedQuery(query, options.queryOptions());
                        queryResult = paginationResult.getResults();
                    } else {
                        queryResult = executeQuery(query, options.queryOptions());
                    }

                    boolean groupsAvailable = serializer.getGroups() != null && !serializer.getGroups().isEmpty();
                    List<Vertex> vertices = groupsAvailable
                            ? queryResult.stream()
                                    .filter(vertex -> OwnerCheck.isAuthorized(options.groups(), vertex))
                    ...
                    int depth = setDepth(binding.version(), obj, params.getFirst("depth"));
                    Format format = null;
                    ...
                    if (method.equals(HttpMethod.PUT)) {
                        ...
                        if (vertices.isEmpty()) {
                            if (options.enableResourceVersion()) {
                    ...
                    switch (method) {
                        case GET:
                            ...
                            if (format == null) {
                                ...
                                FormatFactory ff = new FormatFactory(binding.loader(), serializer, schemaVersions,
                                        basePath + "/", binding.serverBase());
                        ...
                        case PUT:
                            ...
                            if (notificationDepth == AAIProperties.MINIMUM_DEPTH) {
                                ...
                                for (Map.Entry<String, Pair<Introspector, LinkedHashMap<String, Introspector>>> entry : allImpliedDeleteObjs
                                    ...
                                    String curUri = "%s/%s%s".formatted(basePath, binding.version(), entry.getKey());
                                    Introspector curObj = entry.getValue().getValue0();
                                    HashMap<String, Introspector> curObjRelated = entry.getValue().getValue1();
                                    notification.createNotificationEvent(transactionId, options.sourceOfTruth(),
                        ...
                        case PUT_EDGE:
                            ...
                            break;
                        case MERGE_PATCH:
                            Introspector existingObj = binding.loader().introspectorFromName(obj.getDbName());
                            ...
                            try {
                                ...
                                Introspector patchedObj = binding.loader().unmarshal(existingObj.getName(), patched);
                        ...
                        case DELETE:
                            ...
                            if (query.isDependent()) {
                                relatedObjects =
                                        serializer.getRelatedObjects(queryEngine, v, obj, binding.loader());
                            }
                            /*
                            ...
                            List<Vertex> deletableVertices = binding.dbEngine().getQueryEngine().findDeletable(v);
                            Object vId = v.id();
                            ...
                            if (isDelVerticesPresent) {
                                deleteObjects =
                                        this.buildIntrospectorObjects(serializer, deletableVertices, notificationDepth);
                            ...
                            serializer.delete(v, deletableVertices, resourceVersion, options.enableResourceVersion());
                            status = Status.NO_CONTENT;
                            notification.createNotificationEvent(transactionId, options.sourceOfTruth(), status, uri, obj,
                            ...
                            if (isDelVerticesPresent) {
                                notificationService.buildNotificationEvent(options.sourceOfTruth(), status,
                                        transactionId, notification, deleteObjects, uriMap, deleteRelatedObjects,
                                        basePath);
                            }
                            break;
                        case DELETE_EDGE:
                    ...
                    if (response == null && v != null && (method.equals(HttpMethod.PUT) || method.equals(HttpMethod.GET)
                        ...
                        if (paginationResult != null && paginationResult.getTotalCount() != null) {
                            long totalPages = getTotalPages(options.queryOptions(), paginationResult);
                    ...
                    requestResults.add(new RequestResult(request.getUri(), response));
                } catch (JanusGraphException e) {
                    binding.dbEngine().rollback();
        ...
        if (success) {
            notificationService.generateEvents(notification, notificationDepth, options.sourceOfTruth(), serializer,
                    transactionId, queryEngine, mainVertexesToNotifyOn, binding.version());
        } else {
            notification.clearEvents();
        }

        return new ProcessResult(success, requestResults);
    }
    ...
    @Observed(name = "DbRequestProcessor.getObjectFromDb")
    private Introspector getObjectFromDb(List<Vertex> results, DBSerializer serializer, QueryParser query,
    ...
    protected int setDepth(SchemaVersion version, Introspector obj, String depthParam) throws AAIException {
        ...
        if (depthParam == null) {
            if (version.compareTo(schemaVersions.getDepthVersion()) >= 0) {
                depth = 0;
            }
        } else {
    ...
    private Map<String, HashMap<String, Introspector>> buildRelatedObjects(DBSerializer serializer,
            QueryEngine queryEngine, Map<Vertex, Introspector> introSpector, Loader loader) {

        Map<String, HashMap<String, Introspector>> relatedObjectsMap = new HashMap<>();
        for (Map.Entry<Vertex, Introspector> entry : introSpector.entrySet()) {
            try {
                HashMap<String, Introspector> relatedObjects =
                        serializer.getRelatedObjects(queryEngine, entry.getKey(), entry.getValue(), loader);
                if (null != entry.getValue()) {
