...
public class HttpEntry {
    ...
    public Pair<Boolean, List<Pair<URI, Response>>> process(List<DBRequest> requests, String sourceOfTruth,
        ...
        for (DBRequest request : requests) {
            ...
            try {
                response = processRequest(request, serializer, groups, outputMediaType, enableResourceVersion, sourceOfTruth, mainVertexesToNotifyOn, queryOptions);
        ...
        boolean success = !responses.stream()
            .map(pair -> pair.getValue1())
            .anyMatch(response -> response.getStatus() > 300);
        String transactionId = requests.get(0).getTransactionId(); // this should actually be passed in as a parameter
        if (success) {
    ...
    private Response processRequest(DBRequest request, DBSerializer serializer, Set<String> groups, String outputMediaType, boolean enableResourceVersion, String sourceOfTruth, Set<Vertex> mainVertexesToNotifyOn, QueryOptions queryOptions) throws AAIException, UnsupportedEncodingException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, SecurityException, InstantiationException, NoSuchMethodException, URISyntaxException {
        try {
             ...
             */
            Response response;
            if (v != null && (method.equals(HttpMethod.PUT) || method.equals(HttpMethod.GET)
