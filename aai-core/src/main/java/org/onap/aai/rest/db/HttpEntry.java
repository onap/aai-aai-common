...
public class HttpEntry {
    ...
    /**
     * Checks the pagination bucket and pagination index variables to determine whether or not the
     * user
    ...
    /**
     * Setter for the pagination bucket variable which stores in this object the size of results to
     * return
     * 
     * @param pb
     */
    ...
    /**
     * Sets the pagination index that was passed in by the user, to determine which index or results
     * to retrieve when
     * paginated
     * 
     * @param pi
    ...
    /**
     * Sets the total vertices variables and calculates the amount of pages based on size and total
     * vertices
     * 
     * @param totalVertices
     ...
     */
    public Pair<Boolean, List<Pair<URI, Response>>> process(List<DBRequest> requests,
        String sourceOfTruth, boolean enableResourceVersion) throws AAIException {
                ...
                for (retry = 0; retry < maxRetries; ++retry) {
                    try {
                        ...
                        if (this.isPaginated()) {
                            ...
                            vertices = vertTemp.subList(
                                ((this.paginationIndex - 1) * this.paginationBucket),
                                Math.min((this.paginationBucket * this.paginationIndex),
                                    vertTemp.size()));
                        } else {
                        ...
                        String outputMediaType =
                            getMediaType(request.getHeaders().getAcceptableMediaTypes());
                        String result = null;
                        ...
                        List<String> requestContextList =
                            request.getHeaders().getRequestHeader("aai-request-context");
                        ...
                        if (method.equals(HttpMethod.PUT)) {
                            ...
                            if (vertices.isEmpty()) {
                                if (enableResourceVersion) {
                                    serializer.verifyResourceVersion("create",
                                        query.getResultType(), "", resourceVersion, obj.getURI());
                                }
                                isNewVertex = true;
                            } else {
                                if (enableResourceVersion) {
                                    serializer.verifyResourceVersion("update",
                                        query.getResultType(), vertices.get(0)
                                            .<String>property("resource-version").orElse(null),
                            ...
                            if (vertices.isEmpty()) {
                                String msg =
                                    createNotFoundMessage(query.getResultType(), request.getUri());
                        ...
                        switch (method) {
                            case GET:
                                ...
                                if (format == null) {
                                    obj = this.getObjectFromDb(vertices, serializer, query, obj,
                                        request.getUri(), depth, isNodeOnly, cleanUp);
                                    ...
                                    if (obj != null) {
                                        ...
                                        if (!request.getMarshallerProperties().isPresent()) {
                                            properties = new MarshallerProperties.Builder(
                                                org.onap.aai.restcore.MediaType
                                                    .getEnum(outputMediaType)).build();
                                        } else {
                                    ...
                                    FormatFactory ff = new FormatFactory(loader, serializer,
                                        schemaVersions, basePath + "/");
                                    Formatter formatter = ff.get(format, params);
                                    result = formatter
                                        .output(vertices.stream().map(vertex -> (Object) vertex)
                            ...
                            case GET_RELATIONSHIP:
                                if (format == null) {
                                    obj = this.getRelationshipObjectFromDb(vertices, serializer,
                                        query, request.getInfo().getRequestUri());
                                    ...
                                    if (obj != null) {
                                        ...
                                        String msg = createRelationshipNotFoundMessage(
                                            query.getResultType(), request.getUri());
                            ...
                            case PUT:
                                response =
                                    this.invokeExtension(dbEngine, this.dbEngine.tx(), method,
                                        request, sourceOfTruth, version, loader, obj, uri, true);
                                if (isNewVertex) {
                                    v = serializer.createNewVertex(obj);
                                }
                                serializer.serializeToDb(obj, v, query, uri.getRawPath(),
                                    requestContext);
                                this.invokeExtension(dbEngine, this.dbEngine.tx(), HttpMethod.PUT,
                                    request, sourceOfTruth, version, loader, obj, uri, false);
                                status = Status.OK;
                                if (isNewVertex) {
                                ...
                                if (query.isDependent()) {
                                    relatedObjects = this.getRelatedObjects(serializer, queryEngine,
                                        v, obj, this.loader);
                                }
                                LoggingContext.elapsedTime(
                                    (long) serializer.getDBTimeMsecs()
                                        + (long) queryEngine.getDBTimeMsecs(),
                                    TimeUnit.MILLISECONDS);
                                LOGGER.info("Completed ");
                                LoggingContext.restoreIfPossible();
                                notification.createNotificationEvent(transactionId, sourceOfTruth,
                                    status, uri, obj, relatedObjects, basePath);

                                break;
                            case PUT_EDGE:
                                serializer.touchStandardVertexProperties(v, false);
                                this.invokeExtension(dbEngine, this.dbEngine.tx(), method, request,
                                ...
                                notification.createNotificationEvent(transactionId, sourceOfTruth,
                                    ...
                                    new URI(uri.toString()
                                        .replace("/relationship-list/relationship", "")),
                                    serializer.getLatestVersionView(v), relatedObjects, basePath);
                                break;
                            case MERGE_PATCH:
                                Introspector existingObj =
                                    loader.introspectorFromName(obj.getDbName());
                                existingObj = this.getObjectFromDb(vertices, serializer, query,
                                    existingObj, request.getUri(), 0, false, cleanUp);
                                ...
                                Object relationshipList =
                                    request.getIntrospector().getValue("relationship-list");
                                ...
                                try {
                                    ...
                                    Introspector patchedObj =
                                        loader.unmarshal(existingObj.getName(), patched);
                                    if (relationshipList == null) {
                                        // if the caller didn't touch the relationship-list, we
                                        // shouldn't either
                                    ...
                                    if (query.isDependent()) {
                                        relatedObjects = this.getRelatedObjects(serializer,
                                            queryEngine, v, patchedObj, this.loader);
                                    }
                                    LoggingContext.elapsedTime(
                                        (long) serializer.getDBTimeMsecs()
                                            + (long) queryEngine.getDBTimeMsecs(),
                                    ...
                                    LOGGER.info("Completed");
                                    LoggingContext.restoreIfPossible();
                                    notification.createNotificationEvent(transactionId,
                                        sourceOfTruth, status, uri, patchedObj, relatedObjects,
                                        basePath);
                                    ...
                                    throw new AAIException("AAI_3000",
                                        "could not perform patch operation");
                                }
                                break;
                            case DELETE:
                                ...
                                /*
                                 * Find all Delete-other-vertex vertices and create structure for
                                 * notify
                                 * findDeleatble also returns the startVertex v and we dont want to
                                 * create
                                 * duplicate notification events for the same
                                 * So remove the startvertex first
                                 */

                                List<Vertex> deletableVertices =
                                    dbEngine.getQueryEngine().findDeletable(v);
                                Long vId = (Long) v.id();
                                ...
                                if (isDelVerticesPresent) {
                                    deleteObjects = this.buildIntrospectorObjects(serializer,
                                        deletableVertices);

                                    uriMap = this.buildURIMap(serializer, deleteObjects);
                                    deleteRelatedObjects = this.buildRelatedObjects(serializer,
                                        queryEngine, deleteObjects);
                                }

                                this.invokeExtension(dbEngine, this.dbEngine.tx(), method, request,
                                    sourceOfTruth, version, loader, obj, uri, true);
                                serializer.delete(v, deletableVertices, resourceVersion,
                                ...
                                if (isDelVerticesPresent) {
                                    this.buildNotificationEvent(sourceOfTruth, status,
                                        transactionId, notification, deleteObjects, uriMap,
                                        deleteRelatedObjects, basePath);
                                }
                        ...
                        if (response == null && v != null
                            ...
                            if (this.isPaginated()) {
                                response = Response.status(status).header("vertex-id", myvertid)
                                    ...
                                    .entity(result).type(outputMediaType).build();
                            } else {
                                response = Response.status(status).header("vertex-id", myvertid)
                                    .entity(result).type(outputMediaType).build();
                            }
                        ...
                        serializer = new DBSerializer(version, dbEngine, introspectorFactoryType,
                            sourceOfTruth);
                    }
                    if (retry == maxRetries) {
     ...
     */
    private Introspector getObjectFromDb(List<Vertex> results, DBSerializer serializer,
        QueryParser query, Introspector obj, URI uri, int depth, boolean nodeOnly, String cleanUp)
        throws AAIException, IllegalAccessException, IllegalArgumentException,
        InvocationTargetException, SecurityException, InstantiationException, NoSuchMethodException,
        UnsupportedEncodingException, AAIUnknownObjectException, URISyntaxException {
     ...
     * @throws URISyntaxException
     */
    private Introspector getRelationshipObjectFromDb(List<Vertex> results, DBSerializer serializer,
        QueryParser query, URI uri) throws AAIException, IllegalArgumentException,
        SecurityException, UnsupportedEncodingException, AAIUnknownObjectException {
     ...
     * @throws AAIException the AAI exception
     */
    private Response invokeExtension(TransactionalGraphEngine dbEngine, Graph g,
        HttpMethod httpMethod, DBRequest request, String fromAppId, SchemaVersion apiVersion,
        Loader loader, Introspector obj, URI uri, boolean isPreprocess)
        throws IllegalArgumentException, UnsupportedEncodingException, AAIException {
        ...
        ext.runExtension(aaiExtMap.getApiVersion(), extensionInformation.getNamespace(),
            ...
            extensionInformation.getMethodName(httpMethod, isPreprocess), aaiExtMap, isPreprocess);
     ...
     */
    // Legacy support
    private Response notifyOnSkeletonCreation(AAIExtensionMap aaiExtMap, Introspector input,
        HttpHeaders headers) {
        Response response = null;
        ...
        for (AAIResponseMessage msg : aaiExtMap.getPrecheckResponseMessages()
            .getAAIResponseMessage()) {
            ArrayList<String> templateVars = new ArrayList<>();
            ...
            for (AAIResponseMessageDatum dat : msg.getAaiResponseMessageData()
                .getAAIResponseMessageDatum()) {
                keys.add(dat.getAaiResponseMessageDatumKey() + "="
                    + dat.getAaiResponseMessageDatumValue());
            }
    ...
    private String createRelationshipNotFoundMessage(String resultType, URI uri) {
        ...
        String msg = "No relationship found of type " + resultType + " at the given URI: "
            + uri.getPath() + "/relationship-list";
    ...
    private boolean isModificationMethod(HttpMethod method) {
        ...
        if (method.equals(HttpMethod.PUT) || method.equals(HttpMethod.PUT_EDGE)
            || method.equals(HttpMethod.DELETE_EDGE) || method.equals(HttpMethod.MERGE_PATCH)) {
            result = true;
        }
     ...
     * aai-uri ->
     * /cloud-infrastructure/cloud-regions/cloud-region/cloud-owner/cloud-region-id/tenants/tenant/tenant1
     *
     ...
     * @return an array of strings which can be used to get the vertexes of parent and grand parents
     *         from a given vertex
    ...
    String[] convertIntrospectorToUriList(String aaiUri, Introspector obj, Loader loader)
        ...
        while (depth >= 0 && !obj.isTopLevel()) {
            ...
            if (templateCount > truncatedUriCount) {
                LOGGER.warn("Template uri {} contains more slashes than truncatedUri {}", template,
                    truncatedUri);
                return null;
            }

            int cutIndex = StringUtils.ordinalIndexOf(truncatedUri, "/",
                truncatedUriCount - templateCount + 1);
     ...
     * @throws URISyntaxException
     */
    private HashMap<String, Introspector> getRelatedObjects(DBSerializer serializer,
        QueryEngine queryEngine, Vertex v, Introspector obj, Loader loader)
        ...
        if (!aaiUriProperty.isPresent()) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
                    "For the given vertex {}, it seems aai-uri is not present so not getting related objects",
                    v.id().toString());
            } else {
                LOGGER.info(
        ...
        if (!obj.isTopLevel()) {
            ...
            for (Vertex vertex : vertexChain) {
                try {
                    ...
                    LOGGER.warn(
                        "Unable to get vertex properties, partial list of related vertices returned");
                }
            }
        } else {
            try {
    ...
    private Map<Vertex, Introspector> buildIntrospectorObjects(DBSerializer serializer,
    ...
    private Map<String, URI> buildURIMap(DBSerializer serializer,
        Map<Vertex, Introspector> introSpector) {
        Map<String, URI> uriMap = new HashMap<>();
    ...
    private Map<String, HashMap<String, Introspector>> buildRelatedObjects(DBSerializer serializer,
        ...
        for (Map.Entry<Vertex, Introspector> entry : introSpector.entrySet()) {
            try {
                HashMap<String, Introspector> relatedObjects = this.getRelatedObjects(serializer,
                    queryEngine, entry.getKey(), entry.getValue(), this.loader);
                if (null != entry.getValue()) {
    ...
    private void buildNotificationEvent(String sourceOfTruth, Status status, String transactionId,
        UEBNotification notification, Map<Vertex, Introspector> deleteObjects,
        Map<String, URI> uriMap, Map<String, HashMap<String, Introspector>> deleteRelatedObjects,
        String basePath) {
        for (Map.Entry<Vertex, Introspector> entry : deleteObjects.entrySet()) {
            try {
                ...
                if (null != entry.getValue()) {
                    ...
                    if (uriMap.containsKey(vertexObjectId)
                        && deleteRelatedObjects.containsKey(vertexObjectId)) {
                        notification.createNotificationEvent(transactionId, sourceOfTruth, status,
                            uriMap.get(vertexObjectId), entry.getValue(),
                            deleteRelatedObjects.get(vertexObjectId), basePath);
                    }
                }
            } catch (UnsupportedEncodingException | AAIException e) {
    ...
    public void setPaginationParameters(String resultIndex, String resultSize) {
        if (resultIndex != null && resultIndex != "-1" && resultSize != null
            && resultSize != "-1") {
        ...
        if (this.isPaginated()) {
            ...
            if (startIndex > endIndex) {
                throw new AAIException("AAI_6150",
                    " ResultIndex is not appropriate for the result set, Needs to be <= "
                        + endIndex);
            }
