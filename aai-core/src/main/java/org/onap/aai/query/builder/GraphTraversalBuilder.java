...
public abstract class GraphTraversalBuilder<E> extends QueryBuilder<E> {
     ...
     */
    @Override
    public QueryBuilder<Vertex> getVerticesExcludeByProperty(final String key,
        final List<?> values) {
     ...
     */
    @Override
    public QueryBuilder<Vertex> getChildVerticesFromParent(String parentKey, String parentValue,
        String childType) {
            ...
            if (obj.isSimpleType(prop) && !keys.contains(prop)) {
                ...
                if (val != null) {
                    Optional<String> metadata =
                        obj.getPropertyMetadata(prop, PropertyMetadata.DB_ALIAS);
                    if (metadata.isPresent()) {
                        // use the db name for the field rather than the object model
                        prop = metadata.get();
                    }
     ...
     */
    @Override
    public QueryBuilder<Vertex> createEdgeTraversal(EdgeType type, Introspector parent,
    ...
    @Override
    public QueryBuilder<Vertex> createPrivateEdgeTraversal(EdgeType type, Introspector parent,
        Introspector child) throws AAIException {
        this.createTraversal(type, parent, child, true);
        return (QueryBuilder<Vertex>) this;
    }

    private void createTraversal(EdgeType type, Introspector parent, Introspector child,
     ...
     */
    @Override
    public QueryBuilder<Vertex> createEdgeTraversalWithLabels(EdgeType type, Introspector out,
        Introspector in, List<String> labels) throws AAIException {
        this.edgeQueryToVertex(type, out, in, labels);
        return (QueryBuilder<Vertex>) this;
    }

    private Traversal<Vertex, Vertex>[] handleAbstractEdge(EdgeType type,
        Introspector abstractParent, Introspector child, boolean isPrivateEdge)
        throws AAIException {
    ...
    public QueryBuilder<Edge> getEdgesBetweenWithLabels(EdgeType type, String outNodeType,
     ...
     * @throws AAIException
     */
    private void edgeQueryToVertex(EdgeType type, Introspector outObj, Introspector inObj,
        ...
        if (labels == null) {
            ...
            if (rules.isEmpty()) {
                throw new NoEdgeRuleFoundException("No edge rules found for " + outType + " and "
                    + inType + " of type " + type.toString());
            }
        }
     ...
     * @throws AAIException
     */
    private void edgeQuery(EdgeType type, Introspector outObj, Introspector inObj,
        List<String> labels) throws AAIException {
        String outType = outObj.getDbName();
        String inType = inObj.getDbName();
