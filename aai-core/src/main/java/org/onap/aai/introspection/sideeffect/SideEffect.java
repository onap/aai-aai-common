...
public abstract class SideEffect {
    ...
    public SideEffect(Introspector obj, Vertex self, TransactionalGraphEngine dbEngine,
        ...
        this.latestLoader = loaderFactory.createLoaderForVersion(ModelType.MOXY,
    ...
    protected void execute() throws UnsupportedEncodingException, URISyntaxException, AAIException {
        final Map<String, String> properties =
            this.findPopertiesWithMetadata(obj, this.getPropertyMetadata());
        for (Entry<String, String> entry : properties.entrySet()) {
            ...
            try {
                ...
                logger.warn("Unable to execute the side effect {} due to ", e,
                    this.getClass().getName());
            }
        }
    }

    protected Map<String, String> findPopertiesWithMetadata(Introspector obj,
    ...
    protected Map<String, String> findProperties(Introspector obj, String uriString)
        ...
        if (!missing.isEmpty() && (properties != missing.size())) {
            throw new AAIMissingRequiredPropertyException("Cannot complete "
                + this.getPropertyMetadata().toString() + " uri. Missing properties " + missing);
        }
        return result;
    }
    ...
    private Optional<String> resolveRelativePath(Optional<String> populatedUri)
        throws UnsupportedEncodingException {
        if (!populatedUri.isPresent()) {
            return Optional.empty();
        } else {
            return Optional.of(populatedUri.get().replaceFirst("\\./",
                this.serializer.getURIForVertex(self) + "/"));
        }
    }
