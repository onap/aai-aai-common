...
public class IntrospectorValidator implements Wanderer {
        ...
        for (String prop : props) {
            ...
            if (keys.contains(prop)) {
                ...
                if (value == null && validateRequired) {
                    Issue message = this.buildMessage(Severity.CRITICAL,
                        IssueType.MISSING_REQUIRED_PROP, "Missing required property: " + prop);
                    message.setIntrospector(obj);
                    message.setPropName(prop);
                    issues.add(message);
                }
            ...
            final Optional<String> visibility =
                obj.getPropertyMetadata(prop, PropertyMetadata.VISIBILITY);
            if (visibility.isPresent()
                && Visibility.internal.equals(Visibility.valueOf(visibility.get()))
            ...
            final Optional<String> requires =
                obj.getPropertyMetadata(prop, PropertyMetadata.REQUIRES);
            if (requires.isPresent()
                && (obj.getValue(prop) != null && obj.getValue(requires.get()) == null)) {
                Issue message =
                    this.buildMessage(Severity.CRITICAL, IssueType.DEPENDENT_PROP_NOT_FOUND,
     ...
     */
    @Override
    public void modifyComplexList(List<Introspector> list, List<Object> listReference,
        Introspector parent, Introspector child) {
        // NO OP
    }
