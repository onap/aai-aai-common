...
public class PrivateEdge extends SideEffect {
    ...
    public PrivateEdge(Introspector obj, Vertex self, TransactionalGraphEngine dbEngine,
    ...
    protected void processURI(Optional<String> completeUri, Entry<String, String> entry)
        ...
        if (completeUri.isPresent()) {
            ...
            // If it does, then remove the edge since the update happened and doesn't have required
            // props anymore
            // "service-design-and-creation/models/model/{model-invariant-id}/model-vers/model-ver/{model-version-id}"
            // If the vertex does have
            Loader loader = SpringContextAware.getBean(LoaderFactory.class)
                .createLoaderForVersion(ModelType.MOXY, obj.getVersion());
    ...
    private void process(Optional<String> completeUri, Entry<String, String> entry)
        throws URISyntaxException, UnsupportedEncodingException, AAIException,
        EdgeRuleNotFoundException, AmbiguousRuleChoiceException {
        if (completeUri.isPresent()) {
            ...
            if (results.size() == 1) {
                ...
                if (otherVProperty.isPresent()) {
                    ...
                    if (edgeRulesMap.isEmpty()) {
                        String message = String.format("Unable to find edge between %s and %s",
                            obj.getName(), otherVProperty.value().toString());
                        throw new AAIException("AAI_6127", message);
                    } else if (edgeRulesMap.size() > 1) {
                        String message = String.format("Found multiple edges between %s and %s",
                            obj.getName(), otherVProperty.value().toString());
                    ...
                    for (Entry<String, EdgeRule> edgeEntry : edgeRulesMap.entries()) {
                        ...
                        Iterator<Edge> edges =
                            self.edges(edgeRule.getDirection(), edgeRule.getLabel().toString());
                        if (edges.hasNext()) {
                            Edge edge = edges.next();
                            EdgeStatus status = checkStatus(obj, self);
                            switch (status) {
                                case CREATED:
                                    edgeSerializer.addPrivateEdge(
                                        this.dbEngine.asAdmin().getTraversalSource(), self,
                            ...
                            edgeSerializer.addPrivateEdge(
                                this.dbEngine.asAdmin().getTraversalSource(), self, otherVertex,
                                edgeRule.getLabel());
                        }
                    }
                }
    ...
    private EdgeStatus checkStatus(Introspector obj, Vertex self) {
        ...
        for (String key : templateKeys) {
            ...
            if (map.containsKey(PropertyMetadata.DB_ALIAS)) {
                oldVertexValue =
                    self.<String>property(key + AAIProperties.DB_ALIAS_SUFFIX).orElse(null);
            } else {
