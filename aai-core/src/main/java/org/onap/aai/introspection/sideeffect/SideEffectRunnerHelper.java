...
class SideEffectRunnerHelper implements Wanderer {
    ...
    protected SideEffectRunnerHelper(final TransactionalGraphEngine dbEngine,
        final DBSerializer serializer, final Set<Class<? extends SideEffect>> sideEffects) {
    ...
    private void runSideEffects(Introspector obj) throws AAIException {
        for (Class<? extends SideEffect> se : sideEffects) {
            try {
                se.getConstructor(Introspector.class, TransactionalGraphEngine.class,
                    DBSerializer.class).newInstance(obj, dbEngine, serializer).execute();
