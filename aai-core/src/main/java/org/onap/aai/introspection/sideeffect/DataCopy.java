...
public class DataCopy extends SideEffect {
    ...
    public DataCopy(Introspector obj, Vertex self, TransactionalGraphEngine dbEngine,
    ...
    protected void processURI(Optional<String> completeUri, Entry<String, String> entry)
        ...
        if (completeUri.isPresent()) {
            ...
            Introspector resultObj =
                this.latestLoader.introspectorFromName(uriQuery.getResultType());
            if (results.size() == 1) {
                serializer.dbToObject(Collections.singletonList(results.get(0)), resultObj, 0, true,
                    "false");
                try {
                    obj.setValue(entry.getKey(), Objects.requireNonNull(
                        resultObj.getValue(uri.getFragment()), uri.getFragment() + " was null"));
