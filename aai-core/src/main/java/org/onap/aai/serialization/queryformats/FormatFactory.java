...
public class FormatFactory {
    ...
    public FormatFactory(Loader loader, DBSerializer serializer, SchemaVersions schemaVersions,
    ...
    public Formatter get(Format format, MultivaluedMap<String, String> params) throws AAIException {
        ...
        switch (format) {
            ...
            case pathed_resourceversion:
                formatter =
                    new Formatter(inject(new PathedURL(loader, urlBuilder).includeUrl(), params));
                break;
            case id:
            ...
            case resource:
                formatter = new Formatter(
                    inject(new Resource.Builder(loader, serializer, urlBuilder), params).build());
                break;
            case resource_and_url:
                formatter = new Formatter(
                    inject(new Resource.Builder(loader, serializer, urlBuilder).includeUrl(),
                        params).build());
                break;
            case raw:
                formatter = new Formatter(
                    inject(new RawFormat.Builder(loader, serializer, urlBuilder), params).build());
                break;
            case simple:
                formatter = new Formatter(inject(
                    new RawFormat.Builder(loader, serializer, urlBuilder).depth(0).modelDriven(),
                    params).build());
                break;
            case console:
            ...
            case resource_with_sot:
                formatter = new Formatter(
                    inject(new ResourceWithSoT.Builder(loader, serializer, urlBuilder), params)
                        .build());
                break;
            default:
                break;
    ...
    private <T> T inject(T obj, MultivaluedMap<String, String> params)
