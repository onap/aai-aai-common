...
public class AAIGraphTest extends AAISetup{
	...
	public void getRealtimeInstanceConnectionName() throws Exception {
		...
		assertTrue(connectionInstanceName.contains(SERVICE_NAME) && connectionInstanceName.contains("realtime"));
		assertTrue(connectionInstanceName.matches("^\\d+_[\\w\\d]+_" + SERVICE_NAME + "_realtime_\\d+\\(current\\)$"));
		graphMgt.rollback();
	}
	...
	public void getCachedInstanceConnectionName() throws Exception {
		...
		String connectionInstanceName = graphMgt.getOpenInstances().stream().filter(c -> c.contains("current")).findFirst().get();
		assertTrue(connectionInstanceName.contains(SERVICE_NAME) && connectionInstanceName.contains("cached"));
		assertTrue(connectionInstanceName.matches("^\\d+_[\\w\\d]+_" + SERVICE_NAME + "_cached_\\d+\\(current\\)$"));
		graphMgt.rollback();
	}
	...
	public void titanGraphOpenNameTest() throws Exception{
		...
		String connectionInstanceName = graphMgt.getOpenInstances().stream().filter(c -> c.contains("current")).findFirst().get();
		assertTrue(connectionInstanceName.matches("^\\d+_[\\w\\d]+_" + SERVICE_NAME + "_graphType_\\d+\\(current\\)$"));
		graphMgt.rollback();
		graph.close();
	}
