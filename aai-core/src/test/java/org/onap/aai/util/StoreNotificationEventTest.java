...
public class StoreNotificationEventTest extends AAISetup {
    ...
    public void testStoreEventEmptyEventHeader()
        ...
        assertNotNull(res);
        assertTrue(
            res.contains("\"cambria.partition\" : \"" + AAIConstants.UEB_PUB_PARTITION_AAI + "\""));
        assertTrue(res.contains("\"event-header\""));
        assertTrue(res.contains("\"id\""));
        assertTrue(res.contains("\"timestamp\""));
        assertTrue(res.contains("\"source-name\" : \""
            + AAIConfig.get("aai.notificationEvent.default.sourceName") + "\""));
        assertTrue(res.contains(
            "\"domain\" : \"" + AAIConfig.get("aai.notificationEvent.default.domain") + "\""));
        assertTrue(res.contains("\"sequence-number\" : \""
            + AAIConfig.get("aai.notificationEvent.default.sequenceNumber") + "\""));
        assertTrue(res.contains(
            "\"severity\" : \"" + AAIConfig.get("aai.notificationEvent.default.severity") + "\""));
        assertTrue(res.contains("\"event-type\" : \""
            + AAIConfig.get("aai.notificationEvent.default.eventType") + "\""));
        assertTrue(res.contains(
            "\"version\" : \"" + AAIConfig.get("aai.notificationEvent.default.version") + "\""));
    ...
    public void testStoreEvent1Introspector() throws Exception {
        Loader loader = loaderFactory.createLoaderForVersion(ModelType.MOXY,
            schemaVersions.getEdgeLabelVersion());
    ...
    public void testStoreEventIntrospectorEmptyEventHeader() throws Exception {
        ...
        assertTrue(res.contains(
            "\"severity\":\"" + AAIConfig.get("aai.notificationEvent.default.severity") + "\""));
        assertTrue(res.contains(
            "\"event-type\":\"" + AAIConfig.get("aai.notificationEvent.default.eventType") + "\""));
        assertTrue(res.contains(
            "\"version\":\"" + AAIConfig.get("aai.notificationEvent.default.version") + "\""));
        assertTrue(res.contains("\"action\":\"UNK\""));
        assertTrue(res.contains("\"entity-link\":\"UNK\""));
