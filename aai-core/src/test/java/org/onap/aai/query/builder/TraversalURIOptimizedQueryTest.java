 ...
 */
 ...
 public class TraversalURIOptimizedQueryTest extends TraversalQueryTest {
     ...
     public void thatTraversalIsPivotedWithinHasContainer() throws AAIUnknownObjectException {
         ...
         TraversalURIOptimizedQuery traversalQuery = new TraversalURIOptimizedQuery<>(traversal, loader, source, graphTraversalBuilder, stepToAaiUri);
         ...
         assertEquals("[GraphStep(vertex,[]), HasStep([aai-uri.eq(/smth)]), HasStep([propertyKey5.eq(value5)]), RangeGlobalStep(0,3), RangeGlobalStep(0,4)]", query);
     }
     ...
     public void thatTraversalIsPivottedAtRegularStep() throws AAIUnknownObjectException {
         ...
         List<Step> actualStep = rawActual.subList(1, rawActual.size()); // remove GraphStep since I found no way to add it to the expected traversal
