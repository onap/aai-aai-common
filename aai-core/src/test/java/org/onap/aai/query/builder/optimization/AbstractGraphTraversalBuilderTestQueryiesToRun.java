...
public abstract class AbstractGraphTraversalBuilderTestQueryiesToRun
    ...
    public void config() throws Exception {
        ...
        vserverId =
            getPrefix() + "vserver-id-" + tenantNum + "-" + rand.nextInt(getVserverNumPerTenant());
        vserverName = getPrefix() + "vserver-name-" + tenantNum + "-"
            + rand.nextInt(getVserverNumPerTenant());
    ...
    private void callTest(String methodName, int numResultsExpected) throws Exception {
        ...
        Method method = AbstractGraphTraversalBuilderTestQueryiesToRun.class
            .getMethod(queryMethodName, OptimizeEnum.class);
    ...
    public QueryBuilder<Vertex> vserverQuery(OptimizeEnum optimized) throws Exception {
        URI uri = new URI(String.format(vserverUriPattern, getPrefix() + CLOUD_OWNER_1,
            getPrefix() + CLOUD_REGION_ID_1, tenantId, vserverId));
    ...
    public QueryBuilder<Vertex> vserversUnderATenantQuery(OptimizeEnum optimized) throws Exception {
        URI uri = new URI(String.format(tenantUriPattern, getPrefix() + CLOUD_OWNER_1,
            getPrefix() + CLOUD_REGION_ID_1, tenantId) + "/vservers");
        return getQueryBuilder(optimized).createQueryFromURI(uri).getQueryBuilder();
    }
    ...
    public QueryBuilder<Vertex> vserversUnderATenantWithNonIndexPropertyQuery(
        OptimizeEnum optimized) throws Exception {
        URI uri = new URI(String.format(tenantUriPattern, getPrefix() + CLOUD_OWNER_1,
            getPrefix() + CLOUD_REGION_ID_1, tenantId) + "/vservers");
    ...
    public QueryBuilder<Vertex> vserversUnderATenantWithIndexPropertyWhereValueIsInMultipleQuery(
        OptimizeEnum optimized) throws Exception {
        URI uri = new URI(String.format(tenantUriPattern, getPrefix() + CLOUD_OWNER_1,
            getPrefix() + CLOUD_REGION_ID_1, tenantId) + "/vservers");
        MultivaluedMap<String, String> map = new MultivaluedHashMap<>();
    ...
    public QueryBuilder<Vertex> vserversUnderATenantWithIndexPropertyWhereValueIsSemiUniqueQuery(
        OptimizeEnum optimized) throws Exception {
        URI uri = new URI(String.format(tenantUriPattern, getPrefix() + CLOUD_OWNER_1,
            getPrefix() + CLOUD_REGION_ID_1, tenantId) + "/vservers");
        MultivaluedMap<String, String> map = new MultivaluedHashMap<>();
    ...
    public QueryBuilder<Vertex> nonExistentVserverQuery(OptimizeEnum optimized) throws Exception {
        URI uri = new URI(String.format(vserverUriPattern, getPrefix() + CLOUD_OWNER_1,
            getPrefix() + CLOUD_REGION_ID_1, tenantId, "does-not-exist"));
        return getQueryBuilder(optimized).createQueryFromURI(uri).getQueryBuilder();
    }
    ...
    public QueryBuilder<Vertex> vserverWithNonExistentTenantQuery(OptimizeEnum optimized)
        throws Exception {
        URI uri = new URI(String.format(vserverUriPattern, getPrefix() + CLOUD_OWNER_1,
            getPrefix() + CLOUD_REGION_ID_1, "does-not-exist", vserverId));
        return getQueryBuilder(optimized).createQueryFromURI(uri).getQueryBuilder();
    }
    ...
    public QueryBuilder<Vertex> vserverWithValidQueryParameterQuery(OptimizeEnum optimized)
        throws Exception {
        URI uri = new URI(String.format(vserverUriPattern, getPrefix() + CLOUD_OWNER_1,
            getPrefix() + CLOUD_REGION_ID_1, tenantId, vserverId));
        MultivaluedMap<String, String> map = new MultivaluedHashMap<>();
    ...
    public QueryBuilder<Vertex> cloudRegionQuery(OptimizeEnum optimized) throws Exception {
        URI uri = new URI(String.format(crUriPattern, getPrefix() + CLOUD_OWNER_1,
            getPrefix() + CLOUD_REGION_ID_1));
        MultivaluedMap<String, String> map = new MultivaluedHashMap<>();
