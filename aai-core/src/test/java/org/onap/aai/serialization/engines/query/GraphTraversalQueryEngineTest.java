...
public class GraphTraversalQueryEngineTest extends AAISetup {
    ...
    public void testFindParents() throws AAIException {
        ...
        List<Vertex> tenExpected = new ArrayList<>(Arrays.asList(tenant, cloudreg)); // only has the
                                                                                     // immediate
                                                                                     // parent
        List<Vertex> vsExpected = new ArrayList<>(Arrays.asList(vserver, tenant, cloudreg)); // parent
                                                                                             // &
                                                                                             // grandparent
    ...
    public void testFindAllParentsGivenAaiUris() {
        ...
        Vertex cloudreg = graph.addVertex(T.id, "00", "aai-node-type", "cloud-region", "aai-uri",
            "/cloud-infrastructure/cloud-regions/cloud-region/testowner/testid");
        Vertex tenant = graph.addVertex(T.id, "10", "aai-node-type", "tenant", "aai-uri",
            "/cloud-infrastructure/cloud-regions/cloud-region/testowner/testid/tenants/tenant/testTenant1");
        Vertex vserver = graph.addVertex(T.id, "20", "aai-node-type", "vserver", "aai-uri",
        ...
        assertThat("Returned vertices should be 3 cloud region, tenant and vserver",
            vertices.size(), is(3));
    ...
    public void testFindAllChildren() throws AAIException {
        ...
        List<Vertex> crExpected =
            new ArrayList<>(Arrays.asList(cloudreg, tenant, vserver, vserver2, oam));
    ...
    public void testFindRelatedVertices() throws AAIException {
        ...
        // test
        List<Vertex> outRes = engine.findRelatedVertices(gv, Direction.IN,
            "org.onap.relationships.inventory.BelongsTo", "l-interface");
        assertTrue(outRes.containsAll(outExpected) && outExpected.containsAll(outRes));

        List<Vertex> inRes = engine.findRelatedVertices(log, Direction.IN,
            "tosca.relationships.network.LinksTo", "l-interface");
        assertTrue(inRes.containsAll(inExpected) && inExpected.containsAll(inRes));

        List<Vertex> bothRes = engine.findRelatedVertices(lint, Direction.BOTH,
            "tosca.relationships.network.LinksTo", "logical-link");
    ...
    public void testFindSubGraph()
        ...
        List<Element> expected2 =
            new ArrayList<>(Arrays.asList(cr, ten, ten2, vs, vs2, crTen, crTen2, tenVs, tenVs2));
        // findSubGraph(cr)
        List<Element> expected3 = new ArrayList<>(Arrays.asList(cr, ten, ten2, comp, vs, vs2, lint,
            gv, log, crTen, crTen2, crComp, tenVs, tenVs2, vsLInt, vsGv, lintLog));
    ...
    public void testFindEdgesForVersion()
        ...
        Edge cloudRegionToTenantEdge = tn.addEdge("some-edge", cr, CONTAINS.toString(), "NONE");
