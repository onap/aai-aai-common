...
public class SimpleFormatTest extends AAISetup {
    ...
    public void setUp() throws Exception {
        ...
        unknown = graph.addVertex(T.label, "unknown", T.id, "1", "aai-node-type", "unknown",
            "vserver-id", "vserver-id-1", "vserver-name", "vserver-name-1");
    }

    @Test
    public void testCreatePropertiesObjectReturnsProperProperties()
        ...
        serializer = new DBSerializer(schemaVersions.getRelatedLinkVersion(), dbEngine, factoryType,
            "Junit");
        simpleFormat = new RawFormat.Builder(loader, serializer, urlBuilder).nodesOnly(true)
            .depth(0).modelDriven().build();
    ...
    public void testCreatePropertiesObjectThrowsExceptionIfSerializationFails()
        ...
        simpleFormat =
            new RawFormat.Builder(loader, serializer, urlBuilder).nodesOnly(true).depth(0).build();
    ...
    public void createLoaderEngineSetup() {
        ...
        if (loader == null) {
            loader = loaderFactory.createLoaderForVersion(factoryType,
