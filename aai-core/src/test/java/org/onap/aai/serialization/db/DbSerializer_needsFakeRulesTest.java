...
@ContextConfiguration(
    classes = {SchemaLocationsBean.class, SchemaVersions.class,
        AAICoreFakeEdgesConfigTranslator.class, NodeIngestor.class, EdgeIngestor.class,
        EdgeSerializer.class, SpringContextAware.class, IntrospectionConfig.class})
@TestPropertySource(
    properties = {"schema.translator.list = config",
...
public class DbSerializer_needsFakeRulesTest {
    ...
    public void setup() throws Exception {
        ...
        loader = SpringContextAware.getBean(LoaderFactory.class)
            .createLoaderForVersion(introspectorFactoryType, version);
    ...
    public void serializeToDbNewVertexAndEdgeAAIUUIDTest()
        ...
        engine.tx().addVertex("aai-node-type", "vnfc", "vnfc-name", "vnfc-" + testName,
            AAIProperties.AAI_URI, "/network/vnfcs/vnfc/vnfc-" + testName);
        ...
        assertTrue("Edge has uuid ",
            gvnf.edges(Direction.BOTH).next().property(AAIProperties.AAI_UUID).isPresent());

    }

    @Test
    public void createEdgeWithValidLabelTest() throws AAIException, UnsupportedEncodingException,
    ...
    @Test
    public void createEdgeWithValidLabelWhenSameEdgeExistsTest()
        throws AAIException, UnsupportedEncodingException, NoSuchFieldException, SecurityException,
    ...
    @Test
    public void createEdgeWithValidLabelWhenDiffEdgeExistsTest()
        throws AAIException, UnsupportedEncodingException, NoSuchFieldException, SecurityException,
        IllegalArgumentException, IllegalAccessException {
    ...
    @Test
    public void createEdgeWithNoLabelTest() throws AAIException, UnsupportedEncodingException,
        NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
    ...
    @Test
    public void deleteEdgeWithNoLabelWhenMultipleExistsTest()
        throws AAIException, UnsupportedEncodingException, NoSuchFieldException, SecurityException,
        IllegalArgumentException, IllegalAccessException {
        ...
        assertFalse("generic-vnf has no edge uses",
            engine.tx().traversal().V(gvnf).both("uses").hasNext());
        assertFalse("vnfc has no edge uses",
            engine.tx().traversal().V(vnfc).both("uses").hasNext());
        assertTrue("generic-vnf has edge re-uses",
            engine.tx().traversal().V(gvnf).both("re-uses").hasNext());
        assertTrue("vnfc has edge re-uses",
            engine.tx().traversal().V(vnfc).both("re-uses").hasNext());
        assertTrue("generic-vnf has edge re-uses",
            engine.tx().traversal().V(gvnf).both("over-uses").hasNext());
        assertTrue("vnfc has edge re-uses",
            engine.tx().traversal().V(vnfc).both("over-uses").hasNext());
        assertEquals("Number of edges between vertexes is 2", Long.valueOf(2),
            engine.tx().traversal().V(vnfc).both().count().next());
        assertEquals("Number of edges between vertexes is 2", Long.valueOf(2),
            engine.tx().traversal().V(gvnf).both().count().next());
    ...
    @Test
    public void deleteEdgeWithValidLabelWhenMultipleExistsTest()
        throws AAIException, UnsupportedEncodingException, NoSuchFieldException, SecurityException,
        IllegalArgumentException, IllegalAccessException {
    ...
    @Test
    public void serializeToDbWithLabelTest()
        throws AAIException, UnsupportedEncodingException, NoSuchFieldException, SecurityException,
        IllegalArgumentException, IllegalAccessException, URISyntaxException {
        ...
        QueryParser uriQuery = dbEngine.getQueryBuilder()
            .createQueryFromURI(new URI("/network/generic-vnfs/generic-vnf/myvnf"));

        localDbser.serializeToDb(gvnfObj, gvnf, uriQuery, null, "test");

        assertTrue("vertex with vnf-id myvnf exists",
            engine.tx().traversal().V().has("vnf-id", "myvnf").hasNext());
    ...
    @Test
    public void serializeToDbWithoutLabelTest()
        throws AAIException, UnsupportedEncodingException, NoSuchFieldException, SecurityException,
        IllegalArgumentException, IllegalAccessException, URISyntaxException {
    ...
    public void serializeToDbWithInvalidLabelTest()
        ...
        thrown.expectMessage(
            "No EdgeRule found for passed nodeTypes: generic-vnf, vnfc with label NA.");
        localDbser.serializeToDb(gvnfObj, gvnf, uriQuery, null, "test");
    ...
    private DBSerializer getDBSerializerWithSpecificEdgeRules()
        ...
        DBSerializer localDbser =
            new DBSerializer(version, engine, introspectorFactoryType, "AAI-TEST");
        return localDbser;
    }
}
