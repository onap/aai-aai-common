...
@ContextConfiguration(
    classes = {SchemaLocationsBean.class, SchemaVersions.class,
        AAICoreFakeEdgesConfigTranslator.class, EdgeIngestor.class, EdgeSerializer.class})
@DirtiesContext
public class GraphTraversalQueryEngine_needsFakeEdgeRulesTest {
    ...
    public void testFindDeletable() throws AAIException {
        ...
        assertTrue(
            parentExpected.containsAll(parentDeletes) && parentDeletes.containsAll(parentExpected));

        List<Vertex> childDeletes = engine.findDeletable(child);
        assertTrue(
            childExpected.containsAll(childDeletes) && childDeletes.containsAll(childExpected));

        List<Vertex> cousinDeletes = engine.findDeletable(cousin);
        assertTrue(
            cousinExpected.containsAll(cousinDeletes) && cousinDeletes.containsAll(cousinExpected));
    }
}
